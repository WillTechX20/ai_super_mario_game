...
function changeGameStatud(character){
  if(noseXNum!=null&&gameStatusStr=='Game Loading'&&gameConfig.status==="start") {
    world_start.play();
...
function manualControl(character){
  ...
  if(character.live){
    ...
    if(!noseXNum<300&&!noseXNum>300&&!noseYNum<200){ 
...
function jumping(character){
	if( (noseYNum<200&&character.live) || (touchIsDown&&character.live) ){
		mario_jump.play();
...
function startGame(){
  gameStatusStr='Game Loading';
  document.querySelector('#status').innerText='Status: Game Loading';
}
